(() => {
  ...
  const CLEAN_KEY = 'ljs_elec_pwa_clean_version';
  ...
  async function registerServiceWorker() {
    ...
    try {
      ...
      if (!alreadyCleaned) {
        ...
        await Promise.all(registrations.filter(registration => registration.scope.includes('/ljs-elec/')).map(registration => registration.unregister()));
        if ('caches' in window) {
          const names = await caches.keys();
          await Promise.all(names.filter(name => name.startsWith('ljs-elec-')).map(name => caches.delete(name)));
        }
